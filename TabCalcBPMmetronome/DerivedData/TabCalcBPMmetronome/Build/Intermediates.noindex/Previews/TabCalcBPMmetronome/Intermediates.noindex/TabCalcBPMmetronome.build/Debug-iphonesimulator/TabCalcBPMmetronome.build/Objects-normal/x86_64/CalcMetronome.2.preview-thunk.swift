...
extension CalcMetronome {
    @_dynamicReplacement(for: body) private var __preview__body: some View {
        ...
        AnyView(ZStack {
                ...
                VStack{
                    Text(__designTimeString("#20075.[2].[8].property.[0].[0].arg[0].value.[1].arg[0].value.[0].arg[0].value.[0].value", fallback: "BPM変換ツール"))
                        .font(.largeTitle)
                    Text(__designTimeString("#20075.[2].[8].property.[0].[0].arg[0].value.[1].arg[0].value.[1].arg[0].value.[0].value", fallback: "4分音符に換算したときのBPMを計算します"))
                        .font(.title)
                    TextField(__designTimeString("#20075.[2].[8].property.[0].[0].arg[0].value.[1].arg[0].value.[2].arg[0].value.[0].value", fallback: "BPMを入力してください"), text:$BPM)
                        .textFieldStyle(RoundedBorderTextFieldStyle())
                        .keyboardType(.numberPad)
                        .frame(width: __designTimeInteger("#20075.[2].[8].property.[0].[0].arg[0].value.[1].arg[0].value.[2].modifier[2].arg[0].value", fallback: 250))
                    if BPMIntCheck(BPM: BPM){
                        Picker(selection: $Note, label: Text(__designTimeString("#20075.[2].[8].property.[0].[0].arg[0].value.[1].arg[0].value.[3].[0].[0].arg[1].value.arg[0].value.[0].value", fallback: "何分音符ですか？"))) {
                            ForEach(0..<NotesList.count){
                                    Text(self.NotesList[$0])
                            }
                        }
                        Text("\(__designTimeString("#20075.[2].[8].property.[0].[0].arg[0].value.[1].arg[0].value.[3].[0].[1].arg[0].value.[0].value", fallback: ""))\(calcQuarterNotes(BPM: BPM, Notes: NotesList[Note]))\(__designTimeString("#20075.[2].[8].property.[0].[0].arg[0].value.[1].arg[0].value.[3].[0].[1].arg[0].value.[2].value", fallback: "の４分"))")
                                .font(.headline)
                                .foregroundColor(.green)
                        
                        TextField(__designTimeString("#20075.[2].[8].property.[0].[0].arg[0].value.[1].arg[0].value.[3].[0].[2].arg[0].value.[0].value", fallback: "何回鳴らしますか？"), text:$PlayCount)
                        .textFieldStyle(RoundedBorderTextFieldStyle())
                        .keyboardType(.numberPad)
                        .frame(width: __designTimeInteger("#20075.[2].[8].property.[0].[0].arg[0].value.[1].arg[0].value.[3].[0].[2].modifier[2].arg[0].value", fallback: 250))
                        ...
                        Button(action: {
                            ...
                            Text(__designTimeString("#20075.[2].[8].property.[0].[0].arg[0].value.[1].arg[0].value.[3].[0].[3].arg[1].value.[0].arg[0].value.[0].value", fallback: "換算後のBPMを再生"))
                        }
                            
                    }
                    else{
                        Text(__designTimeString("#20075.[2].[8].property.[0].[0].arg[0].value.[1].arg[0].value.[3].[1].[0].arg[0].value.[0].value", fallback: "BPMは10〜1000を入力してください"))
                            .foregroundColor(.red)
                            .font(.headline)
